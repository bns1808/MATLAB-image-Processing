...
Exp_dir{1}= strcat(main_path, '/Experiment_0holes_-5deg_new');
Exp_dir{2}= strcat(main_path, '/Experiment_2holes_-5deg_new');
Exp_dir{3}= strcat(main_path, '/Experiment_4holes_-5deg_new');
...
Exp_dir{2}= strcat(main_path, '/Experiment_0holes_0deg');
% Exp_dir{3}= strcat(main_path, '/Experiment_2holes_2.5deg');
Exp_dir{3}= strcat(main_path, '/Experiment_0holes_5deg_new');
...
threshold = [2.0 2.2 2.4 2.6 2.8 3 3.2 3.4 3.6 3.8 4.0];
...
for exp=1:length(Exp_dir)
    ...
    if exp==2 && density(1)==1010 && elev(1)==0 && option==1
        B=B/1.15;
        Percent(1:end)=D(1:end)/(B(1:end)+D(1:end));
    end
...
for exp=1:length(Exp_dir)
    ...
    if exp==2 && density(2)==1020 && elev(1)==10 && option==2
        B=B/1.15;
        D=D*1.15;
        Percent(1:end)=D(1:end)/(B(1:end)+D(1:end));
    end
...
box on
set(gca,'fontsize',18)
scatter(H(1:end-1),Z(f,1:end-1),S(f,1:end-1),'filled','MarkerfaceColor','red')
scatter(H(end),Z(f,end),S2(f,end),'MarkerEdgeColor','red')


if ess==2 && density(2)==1020 && option==1
    scatter(H(1:end-1),Z2_2(f,1:end-1),S2(f,1:end-1),'filled','MarkerFaceColor','blue')
    scatter(H(end),Z2_2(f,end),S2(f,end),'MarkerEdgeColor','blue')
elseif ess==2
...
x_int=((starting:endings)*30)/T;
    idx=find(x_int>5,1,'first');
    for i=1:length(Exp_dir)
        RR(i) = Sides{i}(idx);
        SS(i) = buoyancies{i}(idx)*T_fix(i);
    end

    if ess==2
        x_int2=((starting:endings)*30)/T2;
        idx=find(x_int2>5,1,'first');
    for i=1:length(Exp_dir)
        RR2(i) = Sides2{i}(idx);
        SS2(i) = buoyancies2{i}(idx)*T_fix2(i);
    end
    end
    ...
    plot(H,RR,'LineWidth', 2,'Color','red')
    hold on; plot(H,RR2,'LineWidth', 2,'Color','blue')

    hold on
    box on
    set(gca,'fontsize',18)
    scatter(H(1:end),RR(1:end),SS(1:end),'filled','MarkerFaceColor','red')

    if ess==2 
        scatter(H(1:end-1),RR2(1:end-1),SS2(1:end-1),'filled','MarkerFaceColor','blue')
        scatter(H(end),RR2(end),SS2(end),'MarkerFaceColor','blue')
    end
...
if ess==2
    ...
    legend({'$\rho = 1.01$ g/cm$^3$', '$\rho = 1.02$ g/cm$^3$'}, 'Interpreter', 'latex','location','northeast');
    end
end
